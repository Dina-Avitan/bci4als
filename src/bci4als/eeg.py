...
class EEG:
    ...
    def get_board_names(self) -> List[str]:
        ...
        if self.headset == "avi13":
            ...
            return ['C3', 'C4', 'Cz', 'FC1', 'FC2', 'FC5', 'FC6', 'CP1', 'CP2', 'CP5', 'CP6']#['CP2', 'FC2', 'CP6', 'C4', 'C3', 'CP5', 'FC1', 'CP1', 'Cz', 'FC6', 'T8', 'T7', 'FC5']
        else:
    ...
    def get_board_channels(self, alternative=True) -> List[int]:
        ...
        if alternative:
            return self.board.get_eeg_channels(self.board_id)[:-5]
        else:
    ...
    def laplacian(data: NDArray):
        ...
        board names are : ['C3', 'C4', 'Cz', 'FC1', 'FC2', 'FC5', 'FC6', 'CP1', 'CP2', 'CP5', 'CP6']
        ...
        names_dict = {'C3': 0, 'C4': 1, 'Cz': 2, 'FC1': 3, 'FC2': 4, 'FC5': 5, 'FC6': 6, 'CP1': 7, 'CP2': 8, 'CP5': 9,
                      'CP6': 10, 'O1': 11, 'O2': 12}
        channel_removed = ['CP5', 'FC5', 'FC1', 'CP1', 'FC2', 'FC6', 'CP2', 'CP6']
        if type(data) == list: #data.shape.__len__() == 3:
            for trial in range(len(data)):
                # C3
                data[trial]['C3'] -= (data[trial]['FC5'] + data[trial]['FC1'] +
                                    data[trial]['CP5'] + data[trial]['CP1']) / 4

                # C4
                data[trial]['C4'] -= (data[trial]['FC2'] + data[trial]['FC6'] +
